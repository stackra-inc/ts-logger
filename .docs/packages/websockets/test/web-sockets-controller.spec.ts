...
class NoopAdapter extends AbstractWsAdapter {
  ...
  public bindMessageHandlers(client: any, handlers, transform: (data: any) => Observable<any>) {
...
describe('WebSocketsController', () => {
  ...
  beforeEach(() => {
    ...
    instance = new WebSocketsController(provider, config, contextCreator as any, graphInspector);
  ...
  describe('connectGatewayToServer', () => {
    ...
    it('should call "subscribeToServerEvents" with default values when metadata is empty', () => {
      ...
      instance.connectGatewayToServer(gateway, DefaultGateway, 'moduleKey', 'instanceWrapperId');
      expect(subscribeToServerEvents.calledWith(gateway, {}, 0, 'moduleKey')).to.be.true;
    });
    it('should call "subscribeToServerEvents" when metadata is valid', () => {
      const gateway = new Test();
      instance.connectGatewayToServer(gateway, Test, 'moduleKey', 'instanceWrapperId');
      expect(subscribeToServerEvents.calledWith(gateway, { namespace }, port, 'moduleKey')).to.be
        .true;
    });
  });
  describe('subscribeToServerEvents', () => {
    ...
    it('should call "assignServerToProperties" with expected arguments', () => {
      ...
      expect(assignServerToProperties.calledWith(gateway, server.server)).to.be.true;
    });
    ...
    describe('when deferredResult contains value which', () => {
      describe('is a Promise', () => {
        it('should return Promise<Observable>', async () => {
          const value = 100;
          expect(
            await lastValueFrom(await instance.pickResult(Promise.resolve(Promise.resolve(value))))
          ).to.be.eq(value);
        });
      });
      ...
      describe('is an Observable', () => {
        it('should return Promise<Observable>', async () => {
          const value = 100;
          expect(
            await lastValueFrom(await instance.pickResult(Promise.resolve(of(value))))
          ).to.be.eq(value);
        });
      });
      ...
      describe('is an object that has the method `subscribe`', () => {
        it('should return Promise<Observable>', async () => {
          ...
          expect(await lastValueFrom(await instance.pickResult(Promise.resolve(value)))).to.equal(
            value
          );
        });
      });
      ...
      describe('is an ordinary value', () => {
        it('should return Promise<Observable>', async () => {
          const value = 100;
          expect(await lastValueFrom(await instance.pickResult(Promise.resolve(value)))).to.be.eq(
            value
          );
        });
      });
