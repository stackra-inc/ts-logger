...
describe('WsContextCreator', () => {
  ...
  beforeEach(() => {
    ...
    exceptionFiltersContext = new ExceptionFiltersContext(new NestContainer() as any);
  ...
  describe('create', () => {
    it('should create exception handler', () => {
      ...
      expect(handlerCreateSpy.calledWith(instance, instance.test as any, module)).to.be.true;
    });
    it('should create pipes context', () => {
      const pipesCreateSpy = sinon.spy(pipesCreator, 'create');
      contextCreator.create(instance, instance.test, module, 'create');
      expect(pipesCreateSpy.calledWith(instance, instance.test, module)).to.be.true;
    });
    ...
    describe('when proxy called', () => {
      it('should call guards consumer `tryActivate`', async () => {
        ...
        sinon.stub(guardsContextCreator, 'create').callsFake(() => [{ canActivate: () => true }]);
        const proxy = contextCreator.create(instance, instance.test, module, 'test');
        const data = 'test';
      ...
      describe('when can not activate', () => {
        it('should throw forbidden exception', () => {
          ...
          const proxy = contextCreator.create(instance, instance.test, module, 'test');
          const data = 'test';
          proxy(null, data).catch((err) => expect(err).to.be.instanceOf(WsException));
        });
      });
    });
  });
