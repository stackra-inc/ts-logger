...
export class SocketsContainer {
  ...
  public getOneByConfig<T extends GatewayMetadata = any>(options: T): ServerAndEventStreamsHost {
    const uniqueToken = this.generateHashByOptions(options);
    return this.serverAndEventStreamsHosts.get(uniqueToken)!;
  }

  public addOne<T extends GatewayMetadata = any>(options: T, host: ServerAndEventStreamsHost) {
    const uniqueToken = this.generateHashByOptions(options);
  ...
  private generateHashByOptions<T extends GatewayMetadata = any>(options: T): string {
