...
export class SocketServerProvider {
  ...
  private createSocketServer<T extends GatewayMetadata>(
    ...
    const { namespace, server, ...partialOptions } = options as Record<string, unknown>;
    const ioServer = adapter.create(port, partialOptions);
    const serverAndEventStreamsHost = ServerAndEventStreamsFactory.create(ioServer);
  ...
  private decorateWithNamespace<T extends GatewayMetadata = any>(
    ...
    const serverAndEventStreamsHost = ServerAndEventStreamsFactory.create(namespaceServer);
  ...
  private getServerOfNamespace<TOptions extends GatewayMetadata = any, TServer = any>(
    options: TOptions,
    port: number,
    server: TServer
  ) {
