...
export class ReplContext {
  ...
  public readonly nativeFunctions = new Map<string, InstanceType<ReplFunctionClass>>();
  ...
  private addNativeFunction(
    ...
    nativeFunction.fnDefinition.aliases?.forEach((aliasName) => {
      const aliasNativeFunction: InstanceType<ReplFunctionClass> = Object.create(nativeFunction);
  ...
  private registerFunctionIntoGlobalScope(nativeFunction: InstanceType<ReplFunctionClass>) {
  ...
  private initializeNativeFunctions(nativeFunctionsClassRefs: ReplFunctionClass[]): void {
