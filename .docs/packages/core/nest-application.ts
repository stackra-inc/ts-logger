...
import { addLeadingSlash, isFunction, isObject, isString } from '@nestjs/common/utils/shared.utils';
  ...
  implements INestApplication
{
  ...
  private readonly microservicesModule = MicroservicesModule && new MicroservicesModule();
  ...
  public connectMicroservice<T extends object>(
    ...
    const { NestMicroservice } = loadPackage('@nestjs/microservices', 'NestFactory', () =>
      require('@nestjs/microservices')
    );
    const { inheritAppConfig } = hybridAppOptions;
    const applicationConfig = inheritAppConfig ? this.config : new ApplicationConfig();
  ...
  public useGlobalFilters(...filters: ExceptionFilter[]): this {
    filters = this.applyInstanceDecoratorIfRegistered<ExceptionFilter>(...filters);
  ...
  public useGlobalPipes(...pipes: PipeTransform<any>[]): this {
    pipes = this.applyInstanceDecoratorIfRegistered<PipeTransform<any>>(...pipes);
  ...
  public useGlobalInterceptors(...interceptors: NestInterceptor[]): this {
    interceptors = this.applyInstanceDecoratorIfRegistered<NestInterceptor>(...interceptors);
  ...
  private applyInstanceDecoratorIfRegistered<T>(...instances: T[]): T[] {
    if (this.appOptions.instrument?.instanceDecorator) {
      return instances.map(
        (instance) => this.appOptions.instrument!.instanceDecorator(instance) as T
      );
    }
    return instances;
  }
