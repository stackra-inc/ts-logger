...
import { addLeadingSlash, isUndefined } from '@nestjs/common/utils/shared.utils';
...
import { ROUTE_MAPPED_MESSAGE, VERSIONED_ROUTE_MAPPED_MESSAGE } from '../helpers/messages';
...
import { Entrypoint, HttpEntrypointMetadata } from '../inspector/interfaces/entrypoint.interface';
  ...
  private applyCallbackToRouter<T extends HttpServer>(
    ...
    paths.forEach((path) => {
      ...
      pathsToRegister.forEach((path) => {
        ...
        const normalizedPath = router.normalizePath ? router.normalizePath(path) : path;
