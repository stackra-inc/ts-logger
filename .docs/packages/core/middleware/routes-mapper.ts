import { MODULE_PATH, PATH_METADATA, VERSION_METADATA } from '@nestjs/common/constants';
import { RouteInfo, Type, VERSION_NEUTRAL, VersionValue } from '@nestjs/common/interfaces';
...
export class RoutesMapper {
  ...
  private getRouteInfoFromController(controller: Type<any>, routePath: string): RouteInfo[] {
    ...
    return ([] as string[])
      .concat(routePath)
      .map((routePath) =>
        controllerPaths.map((item) => toRouteInfo(item, routePath)).reduce(concatPaths, [])
      )
      .reduce(concatPaths, []);
  }
