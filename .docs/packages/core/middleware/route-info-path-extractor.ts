...
import { RouteInfo, VersioningOptions, VersionValue } from '@nestjs/common/interfaces';
import { addLeadingSlash, stripEndSlash } from '@nestjs/common/utils/shared.utils';
...
export class RouteInfoPathExtractor {
  ...
  private extractNonWildcardPathsFrom({ path, method, version }: RouteInfo): string[] {
    const versionPaths = this.extractVersionPathFrom(version);
  ...
  private extractVersionPathFrom(versionValue?: VersionValue): string[] {
    if (!versionValue || this.versioningConfig?.type !== VersioningType.URI) return [];
