...
describe('DebugReplFn', () => {
  ...
  before(async () => {
    ...
    const { moduleRef: aModuleRef } = (await container.addModule(class ModuleA {}, []))!;
    const { moduleRef: bModuleRef } = (await container.addModule(class ModuleB {}, []))!;
    ...
    container.addProvider({ provide: 'StringToken', useValue: 123 }, aModuleRef.token);
  ...
  describe('action', () => {
    it('should print all modules along with their controllers and providers', () => {
      ...
      sinon.stub(replContext, 'writeToStdout').callsFake((text) => (outputText += text));
