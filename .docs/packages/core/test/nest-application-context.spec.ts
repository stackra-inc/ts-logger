...
describe('NestApplicationContext', () => {
  ...
  describe('listenToShutdownSignals', () => {
    it('shutdown process should not be interrupted by another handler', async () => {
      ...
      const waitProcessDown = new Promise((resolve) => {
        ...
        const handler = () => {
          if (!process.listeners(signal).find((handler) => handler == shutdownCleanupRef)) {
            processUp = false;
  ...
  describe('resolve with each: true', () => {
    it('should resolve all default-scoped providers registered under the same token', async () => {
      ...
      const { moduleRef: module1 } = (await nestContainer.addModule(class Module1 {}, []))!;
      const { moduleRef: module2 } = (await nestContainer.addModule(class Module2 {}, []))!;
      const { moduleRef: module3 } = (await nestContainer.addModule(class Module3 {}, []))!;

      nestContainer.addProvider({ provide: TOKEN, useClass: Service1 }, module1.token);
      nestContainer.addProvider({ provide: TOKEN, useClass: Service2 }, module2.token);
      nestContainer.addProvider({ provide: TOKEN, useClass: Service3 }, module3.token);
