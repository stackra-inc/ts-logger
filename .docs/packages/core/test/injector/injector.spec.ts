...
describe('Injector', () => {
  ...
  describe('loadInstance', () => {
    ...
    it('should throw RuntimeException when type is not stored in collection', () => {
      return expect(injector.loadInstance({} as any, moduleDeps.providers, moduleDeps)).to
    ...
    it('should await done$ when "isPending" and rethrow an exception (if thrown)', () => {
      ...
      expect(injector.loadInstance(wrapper, moduleDeps.providers, moduleDeps)).to.eventually.throw(
        error
      );
    });
  ...
  describe('resolveSingleParam', () => {
    it('should throw "RuntimeException" when param is undefined', async () => {
      return expect(
        injector.resolveSingleParam(null!, undefined!, { index: 0, dependencies: [] }, null!)
  ...
  describe('loadMiddleware', () => {
    ...
    it('should not call "loadInstanceSpy" when instance is not resolved', async () => {
      ...
      await injector.loadMiddleware({ metatype: { name: '' } } as any, collection as any, null!);
  ...
  describe('loadController', () => {
    ...
    it('should call "loadInstance" with expected arguments', async () => {
      ...
      expect(loadInstance.calledWith(wrapper, module.controllers, module)).to.be.true;
    });
  });
  ...
  describe('lookupComponentInImports', () => {
    ...
    it('should return null when related modules do not have appropriate component', () => {
      ...
      expect(
        injector.lookupComponentInImports(moduleFixture as any, metatype as any, null!)
  ...
  describe('resolveComponentHost', () => {
    ...
    describe('when instanceWrapper is not resolved and does not have forward ref', () => {
      it('should call loadProvider', async () => {
        ...
        const loadStub = sinon.stub(injector, 'loadProvider').callsFake(() => null!);
    ...
    describe('when instanceWrapper has forward ref and is in non-static context', () => {
      it('should call settlementSignal.error when loadProvider throws', async () => {
        ...
        sinon.stub(injector, 'loadProvider').callsFake(() => Promise.reject(error));
  ...
  describe('resolveConstructorParams', () => {
    ...
    it('should not hang when a parameter is missing @Inject and design:paramtypes is absent', async function () {
      ...
      const container = new NestContainer();
      const { moduleRef } = (await container.addModule(class TestModule {}, []))!;
      moduleRef.addProvider({
        provide: CatService,
        useClass: CatService,
      });
