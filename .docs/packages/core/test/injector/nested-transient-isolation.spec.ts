...
describe('Nested Transient Isolation', () => {
  ...
  describe('when TRANSIENT provider depends on another TRANSIENT provider', () => {
    ...
    it('should create separate TRANSIENT instances for each parent', async () => {
      ...
      await injector.loadInstance(parent1Wrapper, module.providers, module, contextId);
      await injector.loadInstance(parent2Wrapper, module.providers, module, contextId);

      const parent1Instance = parent1Wrapper.getInstanceByContextId(contextId).instance;
      const parent2Instance = parent2Wrapper.getInstanceByContextId(contextId).instance;
    ...
    it('should maintain isolation across multiple request contexts', async () => {
      ...
      await injector.loadInstance(parent1Wrapper, module.providers, module, contextId1);
      await injector.loadInstance(parent2Wrapper, module.providers, module, contextId1);
      await injector.loadInstance(parent1Wrapper, module.providers, module, contextId2);

      const ctx1Parent1 = parent1Wrapper.getInstanceByContextId(contextId1).instance;
      const ctx1Parent2 = parent2Wrapper.getInstanceByContextId(contextId1).instance;
      const ctx2Parent1 = parent1Wrapper.getInstanceByContextId(contextId2).instance;
  ...
  describe('when DEFAULT scoped provider depends on nested TRANSIENT chain', () => {
    ...
    it('should instantiate nested TRANSIENT providers from DEFAULT scope', async () => {
      ...
      expect(parentInstance.transient.nested).to.be.instanceOf(NestedTransientService);
      expect(parentInstance.transient.nested.value).to.equal('nested-initialized');
    });
  });
});
