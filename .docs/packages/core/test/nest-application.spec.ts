...
describe('NestApplication', () => {
  describe('Hybrid Application', () => {
    ...
    it('default should use new ApplicationConfig', () => {
      ...
      expect((instance as any).config.getGlobalInterceptors().length).to.equal(1);
      expect((microservice as any).applicationConfig.getGlobalInterceptors().length).to.equal(0);
    });
    it('should inherit existing ApplicationConfig', () => {
      ...
      expect((instance as any).config.getGlobalInterceptors().length).to.equal(1);
      expect((microservice as any).applicationConfig.getGlobalInterceptors().length).to.equal(1);
    });
