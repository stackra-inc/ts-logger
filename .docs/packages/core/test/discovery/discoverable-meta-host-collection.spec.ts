...
describe('DiscoverableMetaHostCollection', () => {
  ...
  describe('addClassMetaHostLink', () => {
    it('should add a link between a class reference and a metadata key', () => {
      ...
      DiscoverableMetaHostCollection.addClassMetaHostLink(TestClass, metadataKey);

      expect(DiscoverableMetaHostCollection.metaHostLinks.get(TestClass)).to.eq(metadataKey);
    });
    ...
    it('should overwrite existing link for the same class', () => {
      ...
      expect(DiscoverableMetaHostCollection.metaHostLinks.get(TestClass)).to.eq('key2');
    });
  });
  ...
  describe('insertByMetaKey', () => {
    it('should create a new set when metaKey does not exist in collection', () => {
      ...
      DiscoverableMetaHostCollection.insertByMetaKey(metaKey, instanceWrapper, collection);
    ...
    it('should add to existing set when metaKey already exists', () => {
      ...
      DiscoverableMetaHostCollection.insertByMetaKey(metaKey, instanceWrapper1, collection);
      DiscoverableMetaHostCollection.insertByMetaKey(metaKey, instanceWrapper2, collection);
  ...
  describe('getProvidersByMetaKey', () => {
    ...
    it('should return empty set when metaKey is not found', () => {
      ...
      DiscoverableMetaHostCollection.addClassMetaHostLink(TestClass, 'other-key');
      const instanceWrapper = new InstanceWrapper({
        token: TestClass,
        name: 'TestProvider',
        metatype: TestClass,
      });
      DiscoverableMetaHostCollection.inspectProvider(hostContainerRef, instanceWrapper);
  ...
  describe('getControllersByMetaKey', () => {
    ...
    it('should return controllers with matching metaKey', () => {
      ...
      DiscoverableMetaHostCollection.addClassMetaHostLink(TestController, metaKey);
      const instanceWrapper = new InstanceWrapper({
        token: TestController,
        name: 'TestController',
        metatype: TestController,
      });
      DiscoverableMetaHostCollection.inspectController(hostContainerRef, instanceWrapper);
  ...
  describe('inspectProvider', () => {
    ...
    it('should use instance constructor when inject is provided (useFactory)', () => {
      ...
      DiscoverableMetaHostCollection.addClassMetaHostLink(FactoryResultClass, metaKey);
  ...
  describe('inspectController', () => {
    ...
    it('should add controller when metaKey is linked', () => {
      ...
      DiscoverableMetaHostCollection.addClassMetaHostLink(LinkedController, metaKey);
      const instanceWrapper = new InstanceWrapper({
  ...
  describe('inspectInstanceWrapper with existing collection', () => {
    it('should add to existing collection when hostContainerRef is already registered', () => {
      ...
      DiscoverableMetaHostCollection.inspectProvider(hostContainerRef, instanceWrapper1);
      DiscoverableMetaHostCollection.inspectProvider(hostContainerRef, instanceWrapper2);
