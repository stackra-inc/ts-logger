...
class Sink extends Writable implements HeaderStream {
  ...
  constructor(
    public readonly writeHead?: (statusCode: number, headers?: OutgoingHttpHeaders | string) => void
  ) {
...
`
  ...
  it('sets headers on destination when it looks like a HTTP Response', (callback) => {
    const sse = new SseStream();
    const sink = new Sink((status: number, headers: string | OutgoingHttpHeaders) => {
