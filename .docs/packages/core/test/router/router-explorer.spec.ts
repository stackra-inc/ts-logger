...
import { All, Get, Post } from '../../../common/decorators/http/request-mapping.decorator';
...
describe('RouterExplorer', () => {
  ...
  beforeEach(() => {
    ...
    exceptionsFilter = new RouterExceptionFilters(container, applicationConfig, null!);
  ...
  describe('applyPathsToRouterProxy', () => {
    ...
    it('should method return expected object which represents a single versioned route', () => {
      ...
      routerBuilder.applyPathsToRouterProxy(null!, paths as any, null!, '', routePathMetadata, '1');
  ...
  describe('extractRouterPath', () => {
    ...
    it("should throw UnknownRequestMappingException when missing the `@Controller()` decorator in the class, displaying class's name", () => {
      expect(() => routerBuilder.extractRouterPath(ClassWithMissingControllerDecorator)).to.throw(
  ...
  describe('copyMetadataToCallback', () => {
    it('should then copy the metadata from the original callback to the target callback', () => {
      ...
      expect(Reflect.getMetadata('test_metadata_key', targetCallback)).to.be.equal(
        'test_metadata_value'
      );
    });
  });
