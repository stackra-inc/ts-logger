    ...
    describe('when resultOrDeferred', () => {
      describe('is Promise', () => {
        it('should return Promise that resolves to the value resolved by the input Promise', async () => {
          const value = 100;
          expect(await routerResponseController.transformToResult(Promise.resolve(value))).to.be.eq(
            value
          );
        });
      });
      ...
      describe('is Observable', () => {
        it('should return toPromise', async () => {
          const lastValue = 100;
          expect(await routerResponseController.transformToResult(of(1, 2, 3, lastValue))).to.be.eq(
            lastValue
          );
        });
      });
...
`
    ...
    describe('when writing data too densely', () => {
      ...
      it('should not cause memory leak', async () => {
        ...
        Array.from({ length: MAX_LISTENERS + 1 }).forEach((_, i) => result.next(String(i)));
