import { BadRequestException, HttpException, Module, Post, VersioningType } from '@nestjs/common';
...
describe('RoutesResolver', () => {
  ...
  describe('registerRouters', () => {
    it('should register controllers to router instance', () => {
      ...
      const exploreSpy = sinon.spy(untypedRoutesResolver.routerExplorer, 'explore');
      const moduleName = '';
      modules.set(moduleName, {});

      sinon.stub(untypedRoutesResolver.routerExplorer, 'extractRouterPath').callsFake(() => ['']);
      ...
      expect(
        exploreSpy.calledWith(routeWrapper, moduleName, appInstance, undefined, routePathMetadata)
      ).to.be.true;
    });
  ...
  describe('resolve', () => {
    ...
    describe('registerRouters', () => {
      it('should register each module with the base path and append the module path if present ', () => {
        ...
        expect(spy.getCall(0).calledWith(sinon.match.any, sinon.match.any, 'api/v1', '/test')).to.be
          .true;
        expect(
          spy.getCall(1).calledWith(sinon.match.any, sinon.match.any, 'api/v1', sinon.match.any)
        ).to.be.true;
      });
      ...
      it('should register each module with the module path if present', () => {
        ...
        // without module path
        expect(spy.getCall(1).calledWith(sinon.match.any, sinon.match.any, '', undefined)).to.be
          .true;
      });
    });
  });
