...
export class DependenciesScanner {
  ...
  public async scanForModules({
    ...
    moduleDefinition =
      this.getOverrideModuleByModule(moduleDefinition, overrides)?.newModule ?? moduleDefinition;
  ...
  public reflectInjectables(component: Type<Injectable>, token: string, metadataKey: string) {
    const controllerInjectables = this.reflectMetadata<Type<Injectable>>(metadataKey, component);
    const methodInjectables = this.metadataScanner.getAllMethodNames(component.prototype).reduce(
      (acc, method) => {
  ...
  public reflectParamInjectables(component: Type<Injectable>, token: string, metadataKey: string) {
    ...
    paramsMethods.forEach((methodKey) => {
      ...
      params
        ...
        .flat(1)
        .forEach((injectable) =>
          this.insertInjectable(injectable, token, component, 'pipe', methodKey)
        );
    });
  }
    ...
    if (this.isForwardReference(module)) {
      return overrides.find((moduleToOverride) => {
        return (
          ...
          (moduleToOverride.moduleToReplace as ForwardReference).forwardRef?.() ===
            module.forwardRef()
        );
      });
    }
