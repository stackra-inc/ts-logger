  ...
  implements INestApplicationContext
{
  ...
  public select<T>(
    ...
    const token = dynamicMetadata
      ? moduleTokenFactory.createForDynamic(type, dynamicMetadata, moduleType as DynamicModule)
   ...
   * @param {ShutdownHooksOptions} options Options for configuring shutdown hooks behavior
   */
  protected listenToShutdownSignals(signals: string[], options: ShutdownHooksOptions = {}) {
