import { CustomDecorator, flatten, Injectable, SetMetadata } from '@nestjs/common';
import { uid } from 'uid';
...
export class DiscoveryService {
  ...
  static createDecorator<T>(): DiscoverableDecorator<T> {
    ...
    const decoratorFn =
      (opts: T) => (target: object | Function, key?: string | symbol, descriptor?: any) => {
        if (!descriptor) {
  ...
  public getControllers(
    ...
    if ('metadataKey' in options) {
      const controllers = DiscoverableMetaHostCollection.getControllersByMetaKey(
        this.modulesContainer,
        options.metadataKey!
      );
