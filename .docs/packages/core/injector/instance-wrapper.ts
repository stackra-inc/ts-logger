...
import { isClassProvider, isFactoryProvider, isValueProvider } from './helpers/provider-classifier';
...
export class InstanceWrapper<T = any> {
      ...
      (collection, registry) =>
        collection.some(
          (item: InstanceWrapper) =>
            !item.isDependencyTreeStatic() && !item.isDependencyTreeDurable(registry)
        ),
      lookupRegistry
    );
  ...
  public isDependencyTreeStatic(lookupRegistry: string[] = []): boolean {
    ...
    this.isTreeStatic = !this.introspectDepsAttribute(
      (collection, registry) =>
        collection.some((item: InstanceWrapper) => !item.isDependencyTreeStatic(registry)),
      lookupRegistry
    );
