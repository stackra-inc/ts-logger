import { InjectionToken, Logger, LoggerService, OptionalFactoryDependency } from '@nestjs/common';
import {
...
export class Injector {
  ...
  private readonly instanceDecorator: (target: unknown) => unknown = (target: unknown) => target;
  ...
  public getFactoryProviderDependencies<T>(
    ...
    return [wrapper.inject?.map?.(mapFactoryProviderInjectArray) as any[], optionalDependenciesIds];
  }
  ...
  public async lookupComponent<T = any>(
    ...
    if (wrapper && token === name) {
      throw new UnknownDependenciesException(wrapper.name, dependencyContext, moduleRef, {
        id: wrapper.id,
      });
    }
  ...
  public async loadPropertiesMetadata(
    ...
    return dependenciesHosts.map(({ key, host }) => ({
      key,
      name: key,
      instance: host.getInstanceByContextId(this.getContextId(contextId, host), inquirerId)
        .instance,
    }));
  }
  ...
  private printFoundInModuleLog(token: InjectionToken, moduleRef: Module): void {
    ...
    const moduleRefName = moduleRef?.metatype?.name ?? 'unknown';
    this.logger.log(
      `Found ${clc.cyanBright(tokenName)}${clc.green(' in ')}${clc.magentaBright(moduleRefName)}`
    );
  }
