...
import { EnhancerSubtype, GLOBAL_MODULE_METADATA } from '@nestjs/common/constants';
...
export class NestContainer {
  ...
  constructor(
    private readonly _applicationConfig: ApplicationConfig | undefined = undefined,
    private readonly _contextOptions: NestApplicationContextOptions | undefined = undefined
  ) {
    const moduleOpaqueKeyFactory =
      this._contextOptions?.moduleIdGeneratorAlgorithm === 'deep-hash'
        ...
        : new ByReferenceModuleOpaqueKeyFactory({
            keyGenerationStrategy: this._contextOptions?.snapshot ? 'shallow' : 'random',
          });
  ...
  public addExportedProviderOrModule(toExport: Type<any> | DynamicModule, token: string) {
    if (!this.modules.has(token)) {
      throw new UnknownModuleException();
    }
    const moduleRef = this.modules.get(token)!;
