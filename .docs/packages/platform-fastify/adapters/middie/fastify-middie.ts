...
>(
  ...
  function use(
    this: unknown,
    url: string | null | MiddlewareFn<Req, Res, Ctx> | MiddlewareFn<Req, Res, Ctx>[],
  ...
  function run(req: Req, res: Res, ctx: Ctx, initialConfig: FastifyServerOptions | null) {
...
const supportedHooksWithPayload = ['onError', 'onSend', 'preParsing', 'preSerialization'] as const;
...
const supportedHooks = [...supportedHooksWithPayload, ...supportedHooksWithoutPayload] as const;
...
function fastifyMiddie(
  ...
  fastify
    .addHook(
      hook,
      supportedHooksWithPayload.includes(hook as any) ? runMiddieWithPayload : runMiddie
    )
  ...
  function onMiddieEnd(err: unknown, _req: any, _res: any, next: (err?: unknown) => void) {
    next(err);
  }
