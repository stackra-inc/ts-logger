...
export class FastifyAdapter<
  ...
  TRequest extends FastifyRequest<RequestGenericInterface, TServer, TRawRequest> = FastifyRequest<
    RequestGenericInterface,
    TServer,
    TRawRequest
  >,
  TReply extends FastifyReply<RouteGenericInterface, TServer, TRawRequest, TRawResponse> =
    FastifyReply<RouteGenericInterface, TServer, TRawRequest, TRawResponse>,
  TInstance extends FastifyInstance<TServer, TRawRequest, TRawResponse> = FastifyInstance<
    TServer,
    TRawRequest,
    TRawResponse
  >,
> extends AbstractHttpAdapter<TServer, TRequest, TReply> {
  ...
  private readonly versionConstraint = {
    ...
    storage() {
      ...
      return {
        ...
        set(versionOrVersions: string | Array<string>, store: unknown) {
          const storeVersionConstraint = (version: string) => versions.set(version, store);
          if (Array.isArray(versionOrVersions)) versionOrVersions.forEach(storeVersionConstraint);
    ...
    deriveConstraint: (req: FastifyRequest) => {
      ...
      if (this.versioningOptions?.type === VersioningType.MEDIA_TYPE) {
        ...
        const acceptHeaderValue: string | undefined = (req.headers?.[MEDIA_TYPE_HEADER] ||
  ...
  public setOnRequestHook(
    hook: (request: TRequest, reply: TReply, done: (err?: Error) => void) => void | Promise<void>
  ) {
  ...
  public reply(response: TRawResponse | TReply, body: any, statusCode?: number) {
  ...
  public render(response: TReply & { view: Function }, view: string, options: any) {
  ...
  public enableCors(options?: FastifyCorsOptions) {
    this.register(import('@fastify/cors') as Parameters<TInstance['register']>[0], options);
  }
  ...
  public registerParserMiddleware(prefix?: string, rawBody?: boolean) {
    ...
    this._pathPrefix = prefix ? (!prefix.startsWith('/') ? `/${prefix}` : prefix) : undefined;
  }
  ...
  public useBodyParser(
    ...
    this.getInstance().addContentTypeParser<Buffer>(
      type,
      parserOptions,
      (req: RawBodyRequest<FastifyRequest<any, TServer, TRawRequest>>, body: Buffer, done) => {
    ...
    return (path: string, callback: Function) => {
      ...
      try {
        ...
        this.instance.use(normalizedPath, (req: any, res: any, next: Function) => {
          ...
          let pathname =
            queryParamsIndex >= 0 ? req.originalUrl.slice(0, queryParamsIndex) : req.originalUrl;
  ...
  private isNativeResponse(response: TRawResponse | TReply): response is TRawResponse {
  ...
  private injectRouteOptions(routerMethodKey: Uppercase<HTTPMethods>, ...args: any[]) {
    ...
    const routeToInject: RouteOptions<TServer, TRawRequest, TRawResponse> & RouteShorthandOptions =
      {
