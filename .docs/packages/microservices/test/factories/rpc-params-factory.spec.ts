    ...
    describe('when key is', () => {
      ...
      describe(`RpcParamtype.PAYLOAD`, () => {
        it('should return a message payload object', () => {
          expect(factory.exchangeKeyForValue(RpcParamtype.PAYLOAD, null!, args)).to.be.eql(payload);
        });
        it('should return a message payload object with parameter extraction', () => {
          expect(factory.exchangeKeyForValue(RpcParamtype.PAYLOAD, 'data', args)).to.be.eql(
            payload.data
          );
        });
      });
      describe(`RpcParamtype.CONTEXT`, () => {
        it('should return a ctx object', () => {
          expect(factory.exchangeKeyForValue(RpcParamtype.CONTEXT, null!, args)).to.be.eql(ctx);
        });
      });
    });
