...
import { PATTERN_EXTRAS_METADATA, PATTERN_METADATA, TRANSPORT_METADATA } from '../../constants';
...
describe('@EventPattern', () => {
  ...
  it(`should enhance method with ${PATTERN_EXTRAS_METADATA} metadata`, () => {
    const metadata = Reflect.getMetadata(PATTERN_EXTRAS_METADATA, TestComponent.test);
    expect(metadata).to.be.deep.equal(extras);
  });
  it(`should enhance method with last ${PATTERN_METADATA} metadata`, () => {
    const metadata = Reflect.getMetadata(PATTERN_METADATA, TestComponent.testOnlyThird);
    expect(metadata.length).to.equal(1);
    expect(metadata[0]).to.be.eql(patternSecond);
  });
  it(`should enhance method with both ${PATTERN_METADATA} metadata`, () => {
    const metadata = Reflect.getMetadata(PATTERN_METADATA, TestComponent.testBoth);
  ...
  describe('decorator overloads', () => {
    ...
    class TestComponent5 {
      ...
      @((
        (): MethodDecorator => (_target, _key, descriptor) =>
          Reflect.defineMetadata(PATTERN_EXTRAS_METADATA, additionalExtras, descriptor.value!)
      )())
      public static test() {}
    }

    it(`should enhance method with ${PATTERN_METADATA} metadata`, () => {
      const [metadataArg] = Reflect.getMetadata(PATTERN_METADATA, TestComponent1.test);
      const transportArg = Reflect.getMetadata(TRANSPORT_METADATA, TestComponent1.test);
      const extrasArg = Reflect.getMetadata(PATTERN_EXTRAS_METADATA, TestComponent1.test);
    ...
    it(`should enhance method with ${PATTERN_METADATA}, ${TRANSPORT_METADATA} metadata`, () => {
      const [metadataArg] = Reflect.getMetadata(PATTERN_METADATA, TestComponent2.test);
      const transportArg = Reflect.getMetadata(TRANSPORT_METADATA, TestComponent2.test);
      const extrasArg = Reflect.getMetadata(PATTERN_EXTRAS_METADATA, TestComponent2.test);
      expect(metadataArg).to.be.eql(pattern);
    ...
    it(`should enhance method with ${PATTERN_METADATA}, ${PATTERN_EXTRAS_METADATA} metadata`, () => {
      const [metadataArg] = Reflect.getMetadata(PATTERN_METADATA, TestComponent3.test);
      const transportArg = Reflect.getMetadata(TRANSPORT_METADATA, TestComponent3.test);
      const extrasArg = Reflect.getMetadata(PATTERN_EXTRAS_METADATA, TestComponent3.test);
      expect(metadataArg).to.be.eql(pattern);
      expect(transportArg).to.be.undefined;
...
${PATTERN_EXTRAS_METADATA} metadata`, () => {
      const [metadataArg] = Reflect.getMetadata(PATTERN_METADATA, TestComponent4.test);
      const transportArg = Reflect.getMetadata(TRANSPORT_METADATA, TestComponent4.test);
      const extrasArg = Reflect.getMetadata(PATTERN_EXTRAS_METADATA, TestComponent4.test);
      expect(metadataArg).to.be.eql(pattern);
      expect(transportArg).to.be.eql(Transport.TCP);
      expect(extrasArg).to.be.eql(extras);
    });
    ...
    it(`should merge with existing ${PATTERN_EXTRAS_METADATA} metadata`, () => {
      const [metadataArg] = Reflect.getMetadata(PATTERN_METADATA, TestComponent5.test);
      const transportArg = Reflect.getMetadata(TRANSPORT_METADATA, TestComponent5.test);
      const extrasArg = Reflect.getMetadata(PATTERN_EXTRAS_METADATA, TestComponent5.test);
      expect(metadataArg).to.be.eql(pattern);
      expect(transportArg).to.be.eql(Transport.TCP);
      expect(extrasArg).to.be.eql({
        ...additionalExtras,
