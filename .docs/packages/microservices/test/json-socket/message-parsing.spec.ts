...
describe('JsonSocket message parsing', () => {
  ...
  it('should parse multiple messages with unicode correctly', () => {
    ...
    expect(messages[0]).to.deep.equal('Diese Zeile enthält das Unicode-Zeichen');
    expect(messages[1]).to.deep.equal(true);
    expect(socket['buffer']).to.deep.equal('');
  });
  ...
  describe('Error handling', () => {
    describe('JSON Error', () => {
      ...
      it(`should emit ${TcpEventsMap.ERROR} event on socket`, () => {
        const socketEmitSpy: sinon.SinonSpy<any, any> = sinon.spy(socket['socket'], 'emit');

        socket['onData'](packet);

        try {
          expect(socketEmitSpy.calledOnceWithExactly(TcpEventsMap.ERROR, errorMsg)).to.be.true;
        } catch (err) {
          expect(socketEmitSpy.calledWithExactly(TcpEventsMap.ERROR, errorMsgNodeAboveV20)).to.be
            .true;
        } finally {
