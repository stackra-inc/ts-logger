...
describe('JsonSocket chaining', () => {
  it('should return the instance when subscribing to event', (done) => {
    helpers.createServerAndClient((err, server, clientSocket, serverSocket) => {
      ...
      expect(clientSocket!.on(MESSAGE_EVENT, () => {})).to.be.instanceof(JsonSocket);
      expect(clientSocket!.on(TcpEventsMap.CONNECT, () => {})).to.deep.equal(clientSocket);
      expect(clientSocket!.on(MESSAGE_EVENT, () => {}).on('end', () => {})).to.deep.equal(
        clientSocket
      );
