...
describe('ServerRMQ', () => {
  ...
  describe('listen', () => {
    ...
    beforeEach(() => {
      onStub = sinon.stub().callsFake((event, callback) => event === 'connect' && callback());
      createChannelStub = sinon.stub().callsFake(({ setup }) => setup());
      setupChannelStub = sinon.stub(server, 'setupChannel').callsFake(() => ({}) as any);
  ...
  describe('setupChannel', () => {
    ...
    it('should call "assertQueue" with queue and queue options when queue is default queue', async () => {
      ...
      await server.setupChannel(channel, () => null);
      expect(channel.assertQueue.calledWith(RQM_DEFAULT_QUEUE, queueOptions)).to.be.true;
    });
    ...
    it('should call "prefetch" with prefetchCount and "isGlobalPrefetchCount"', async () => {
      await server.setupChannel(channel, () => null);
      expect(channel.prefetch.calledWith(prefetchCount, isGlobalPrefetchCount)).to.be.true;
    });
  ...
  describe('sendMessage', () => {
    ...
    it('should publish message to indicated queue', () => {
      ...
      expect(
        channel.sendToQueue.calledWith(replyTo, Buffer.from(JSON.stringify(message)), {
          correlationId,
        })
      ).to.be.true;
    });
  ...
  describe('handleEvent', () => {
    ...
    it('should call handler with expected arguments', async () => {
      ...
      await server.handleEvent(channel, { pattern: '', data }, new RmqContext([{}, {}, '']));
    ...
    it('should negative acknowledge without retrying if key does not exists in handlers object and noAck option is false', async () => {
      ...
      untypedServer.noAck = false;
      await server.handleEvent(channel, message, new RmqContext([message, '', '']));
  ...
  describe('matchRmqPattern', () => {
    ...
    describe('exact matches', () => {
      ...
      it('should handle patterns with $ character (original issue)', () => {
        expect(matchRmqPattern('$internal.plugin.status', '$internal.plugin.status')).to.be.true;
        expect(matchRmqPattern('$internal.plugin.0.status', '$internal.plugin.0.status')).to.be
          .true;
        expect(matchRmqPattern('user.$special.event', 'user.$special.event')).to.be.true;
      });
    });
    ...
    describe('single wildcard (*)', () => {
      ...
      it('should not match when segment counts differ', () => {
        ...
        expect(matchRmqPattern('*.created', 'user.profile.created')).to.be.false;
      });

      it('should handle patterns with $ and *', () => {
        expect(matchRmqPattern('$internal.plugin.*.status', '$internal.plugin.0.status')).to.be
          .true;
        expect(matchRmqPattern('$internal.plugin.*.status', '$internal.plugin.1.status')).to.be
          .true;
        expect(matchRmqPattern('$internal.*.status', '$internal.plugin.status')).to.be.true;
      });

      it('should handle multiple * wildcards', () => {
        expect(matchRmqPattern('*.*.created', 'user.profile.created')).to.be.true;
    ...
    describe('catch all wildcard (#)', () => {
      it('should match when # is at the end', () => {
        ...
        expect(matchRmqPattern('user.#', 'user.profile.details.updated')).to.be.true;
      });

      it('should handle patterns with $ and #', () => {
        expect(matchRmqPattern('$internal.#', '$internal.plugin.status')).to.be.true;
        expect(matchRmqPattern('$internal.#', '$internal.plugin.0.status')).to.be.true;
        expect(matchRmqPattern('$internal.plugin.#', '$internal.plugin.0.status')).to.be.true;
      });
    ...
    describe('edge cases', () => {
      ...
      it('should handle complex $ patterns that previously failed', () => {
        expect(matchRmqPattern('$exchange.*.routing.#', '$exchange.topic.routing.key.test')).to.be
          .true;
        expect(matchRmqPattern('$sys.#', '$sys.broker.clients')).to.be.true;
        expect(matchRmqPattern('$SYS.#', '$SYS.broker.load.messages.received')).to.be.true;
      });
    });
  });
});
