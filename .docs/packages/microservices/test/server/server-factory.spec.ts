...
describe('ServerFactory', () => {
  describe('create', () => {
    ...
    it(`should return redis server`, () => {
      expect(ServerFactory.create({ transport: Transport.REDIS }) instanceof ServerRedis).to.be
        .true;
    });
    ...
    it(`should return mqtt server`, () => {
      expect(ServerFactory.create({ transport: Transport.MQTT }) instanceof ServerMqtt).to.be.true;
    });

    it(`should return nats server`, () => {
      expect(ServerFactory.create({ transport: Transport.NATS }) instanceof ServerNats).to.be.true;
    });

    it(`should return rmq server`, () => {
      expect(ServerFactory.create({ transport: Transport.RMQ }) instanceof ServerRMQ).to.be.true;
    });

    it(`should return kafka server`, () => {
      expect(ServerFactory.create({ transport: Transport.KAFKA }) instanceof ServerKafka).to.be
        .true;
    });
