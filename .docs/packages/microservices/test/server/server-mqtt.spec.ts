...
describe('ServerMqtt', () => {
  ...
  describe('getMessageHandler', () => {
    it(`should return function`, () => {
      expect(typeof server.getMessageHandler(untypedServer.mqttClient)).to.be.eql('function');
    });
    describe('handler', () => {
      it('should call "handleMessage"', async () => {
        const handleMessageStub = sinon.stub(server, 'handleMessage').callsFake(() => null!);
        await server.getMessageHandler(untypedServer.mqttClient)(null!, null!, null!);
  ...
  describe('handleMessage', () => {
    ...
    it('should call "handleEvent" if identifier is not present', async () => {
      ...
      await server.handleMessage(channel, Buffer.from(JSON.stringify({ pattern: '', data })), null);
  ...
  describe('getPublisher', () => {
    ...
    it(`should return function`, () => {
      expect(typeof server.getPublisher(null, context, id)).to.be.eql('function');
    });
    it(`should call "publish" with expected arguments`, () => {
      const respond = 'test';
      publisher({ respond, id });
      expect(publisherSpy.calledWith(`${pattern}/reply`, JSON.stringify({ respond, id }))).to.be
        .true;
    });
  });
  ...
  describe('parseMessage', () => {
    it(`should return parsed json`, () => {
      const obj = { test: 'test' };
      expect(server.parseMessage(obj)).to.deep.equal(JSON.parse(JSON.stringify(obj)));
    });
  ...
  describe('handleEvent', () => {
    ...
    it('should call handler with expected arguments', async () => {
      ...
      await server.handleEvent(channel, { pattern: '', data }, new BaseRpcContext([]));
      expect(handler.calledWith(data)).to.be.true;
    });
  });
  describe('matchMqttPattern', () => {
    ...
    it('should return false when topic does not matches with provided pattern', () => {
      ...
      expect(server.matchMqttPattern('root/test/#', 'root/invalid/child')).to.be.false;
      expect(server.matchMqttPattern('root/#/grandchild', 'root/invalid/child/grandchild')).to.be
        .false;
      expect(server.matchMqttPattern('root/+/grandchild', 'root/invalid/child/grandchild')).to.be
        .false;
    });
