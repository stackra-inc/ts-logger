...
describe('ServerNats', () => {
  ...
  describe('close', () => {
    ...
    describe('when "gracefulShutdown" is true', () => {
      ...
      const subscriptions = [{ unsubscribe: sinon.spy() }, { unsubscribe: sinon.spy() }];
      beforeEach(() => {
  ...
  describe('getPublisher', () => {
    ...
    it(`should return function`, () => {
      ...
      expect(typeof server.getPublisher(natsMsg, id, context)).to.be.eql('function');
    });
    it(`should call "respond" when reply topic provided`, () => {
      ...
      expect(natsMsg.respond.calledWith(JSONCodec().encode({ respond, id }))).to.be.true;
    });
