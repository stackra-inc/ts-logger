...
describe('ServerGrpc', () => {
  ...
  describe('listen', () => {
    ...
    beforeEach(() => {
      callback = sinon.spy();
      bindEventsStub = sinon.stub(server, 'bindEvents').callsFake(() => ({}) as any);
    });
  ...
  describe('listen (multiple proto)', () => {
    ...
    beforeEach(() => {
      callback = sinon.spy();
      bindEventsStub = sinon.stub(serverMulti, 'bindEvents').callsFake(() => ({}) as any);
    });
  ...
  describe('createService', () => {
    const objectToMap = (obj) => new Map(Object.keys(obj).map((key) => [key, obj[key]]) as any);
    ...
    it('should call "createServiceMethod"', async () => {
      ...
      const spy = sinon.stub(server, 'createServiceMethod').callsFake(() => ({}) as any);
    ...
    describe('when RX streaming', () => {
      it('should call "createPattern" with proper arguments', async () => {
        ...
        expect(createPatternStub.calledWith('name', 'test2', GrpcMethodStreamingType.RX_STREAMING))
          .to.be.true;
      });
    });
    describe('when pass through streaming', () => {
      it('should call "createPattern" with proper arguments', async () => {
        ...
        expect(createPatternStub.calledWith('name', 'test2', GrpcMethodStreamingType.PT_STREAMING))
          .to.be.true;
      });
    });
  });
  ...
  describe('getMessageHandler', () => {
    it('should return handler when service name specified', () => {
      ...
      expect(
        server.getMessageHandler('test', 'TestMethod', GrpcMethodStreamingType.NO_STREAMING, {})
      ).not.to.be.undefined;
    });
  ...
  describe('createPattern', () => {
    it('should return pattern', () => {
      const service = 'test';
      const method = 'method';
      expect(server.createPattern(service, method, GrpcMethodStreamingType.NO_STREAMING)).to.be.eql(
        JSON.stringify({
          service,
          rpc: method,
    ...
    describe('when response is not a stream', () => {
      ...
      describe('when response is a stream', () => {
        ...
        async function createCall() {
          ...
          const serviceMethod = server.createRequestStreamMethod(methodHandler, true);
  ...
  describe('loadProto', () => {
    describe('when proto is invalid', () => {
      it('should throw InvalidProtoDefinitionException', () => {
        const getPackageDefinitionStub = sinon.stub(grpcHelpers, 'getGrpcPackageDefinition' as any);
        getPackageDefinitionStub.callsFake(() => {
          throw new Error();
        });
        untypedServer.logger = new NoopLogger();
        expect(() => server.loadProto()).to.throws(InvalidProtoDefinitionException);
  ...
  describe('deserialize', () => {
    it(`should return parsed json`, () => {
      const obj = { test: 'test' };
      expect(server.deserialize(obj)).to.deep.equal(JSON.parse(JSON.stringify(obj)));
    });
  ...
  describe('addHandler', () => {
    ...
    it(`should add handler`, () => {
      ...
      const messageHandlersSetSpy = sinon.spy(untypedServer.messageHandlers, 'set');
      server.addHandler(pattern, callback as any);

      expect(messageHandlersSetSpy.called).to.be.true;
      expect(messageHandlersSetSpy.getCall(0).args[0]).to.be.equal(JSON.stringify(pattern));
    });
  });
});
