...
import { EachMessagePayload, KafkaMessage } from '../../external/kafka.interface';
...
describe('ServerKafka', () => {
  ...
  describe('getPublisher', () => {
    ...
    beforeEach(() => {
      publisher = server.getPublisher(replyTopic, replyPartition, correlationId, context);
      sendMessageStub = sinon.stub(server, 'sendMessage').callsFake(async () => []);
    });
    it(`should return function`, () => {
      expect(typeof server.getPublisher(null!, null!, correlationId, context)).to.be.eql(
        'function'
      );
    });
    it(`should call "publish" with expected arguments`, () => {
      ...
      expect(sendMessageStub.calledWith(data, replyTopic, replyPartition, correlationId)).to.be
        .true;
    });
  });
  ...
  describe('handleMessage', () => {
    ...
    it('should call event handler when "handleEvent" is called', async () => {
      ...
      await server.handleEvent(topic, { data: messageData, pattern: topic }, context);
    ...
    it('should not catch error thrown by event handler as part of "handleEvent"', async () => {
      ...
      try {
        await server.handleEvent(topic, { data: 'some data', pattern: topic }, {} as any);
