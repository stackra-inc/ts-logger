...
describe('ClientKafka', () => {
  ...
  describe('emitBatch', () => {
    ...
    describe('when is connected', () => {
      beforeEach(() => {
        sinon.stub(client, 'connect').callsFake(() => Promise.resolve({} as Producer));
      });

      it(`should call dispatchBatchEvent`, () => {
        const pattern = { test: 3 };
        const data = { messages: [] };
        const dispatchBatchEventSpy = sinon.stub().callsFake(() => Promise.resolve(true));
  ...
  describe('dispatchEvent', () => {
    ...
    it('should throw error', async () => {
      ...
      client['dispatchEvent'](eventMessage).catch((err) => expect(err).to.be.instanceOf(Error));
    });
  });
  ...
  describe('publish', () => {
    const waitForNextTick = async () => await new Promise((resolve) => process.nextTick(resolve));
    const readPacket = {
      pattern: topic,
      data: messageValue,
    };
    ...
    beforeEach(() => {
      ...
      assignPacketIdStub = sinon.stub(client as any, 'assignPacketId').callsFake((packet) =>
