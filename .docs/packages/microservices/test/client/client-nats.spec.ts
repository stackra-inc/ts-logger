...
describe('ClientNats', () => {
  ...
  describe('publish', () => {
    ...
    beforeEach(() => {
      ...
      createClient = sinon.stub(client, 'createClient').callsFake(() => untypedClient);
    });
    afterEach(() => {
      connectSpy.restore();
    ...
    describe('on error', () => {
      ...
      beforeEach(() => {
        assignPacketIdStub = sinon.stub(client, 'assignPacketId' as any).callsFake(() => {
          throw new Error();
        });
      });
      afterEach(() => {
    ...
    describe('headers', () => {
      ...
      it('should send packet headers', () => {
        ...
        client['publish'](msg, () => {});
        expect(natsClient.publish.getCall(0).args[2].headers.get('1')).to.eql('123');
      });
      it('should combine packet and static headers', () => {
        ...
        client['publish'](msg, () => {});
        expect(publishSpy.getCall(0).args[2].headers.get('client-id')).to.eql('some-client-id');
      ...
      it('should prefer packet headers over static headers', () => {
        ...
        client['publish'](msg, () => {});
        expect(publishSpy.getCall(0).args[2].headers.get('client-id')).to.eql('override-client-id');
      });
    });
  });
  ...
  describe('connect', () => {
    ...
    beforeEach(async () => {
      createClientSpy = sinon.stub(client, 'createClient').callsFake(() => Promise.resolve({}));
  ...
  describe('handleStatusUpdates', () => {
    ...
    it('should log "disconnect" and "error" statuses as "errors"', async () => {
      ...
      expect(logErrorSpy.calledWith(`NatsError: type: "disconnect", data: "localhost".`));
      expect(logErrorSpy.calledWith(`NatsError: type: "disconnect", data: "{}".`));
    });
    it('should log other statuses as "logs"', async () => {
      ...
      expect(logSpy.calledWith(`NatsStatus: type: "non-disconnect", data: "localhost".`));
  ...
  describe('dispatchEvent', () => {
    ...
    beforeEach(() => {
      ...
      subscribeStub = sinon.stub().callsFake((channel, options) => options.callback());
      natsClient = {
        publish: sinon.spy(),
    ...
    it('should throw error', async () => {
      subscribeStub.callsFake((channel, options) => options.callback(new Error()));
      await client['dispatchEvent'](msg).catch((err) => expect(err).to.be.instanceOf(Error));
    });
    ...
    describe('headers', () => {
      ...
      it('should combine packet and static headers', async () => {
        ...
        await client['dispatchEvent'](msg);
        expect(natsClient.publish.getCall(0).args[2].headers.get('client-id')).to.eql(
          'some-client-id'
        );
        expect(natsClient.publish.getCall(0).args[2].headers.get('1')).to.eql('123');
      });
