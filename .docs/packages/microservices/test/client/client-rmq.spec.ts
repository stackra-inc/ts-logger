...
describe('ClientRMQ', function () {
  ...
  describe('connect', () => {
    ...
    beforeEach(async () => {
      ...
      sinon.stub(client, 'mergeDisconnectEvent').callsFake((_, source) => source);
    });
  ...
  describe('createChannel', () => {
    ...
    beforeEach(() => {
      setupChannelStub = sinon.stub(client, 'setupChannel').callsFake((_, done) => done());
  ...
  describe('consumeChannel', () => {
    ...
    beforeEach(() => {
      ...
      consumeStub = sinon.stub().callsFake((_, done) => done({ properties: { correlationId: 1 } }));
  ...
  describe('publish', () => {
    ...
    it('should send buffer from stringified message', () => {
      client['publish'](msg, () => {
        expect(sendToQueueStub.called).to.be.true;
        expect(sendToQueueStub.getCall(1).args[1]).to.be.eql(Buffer.from(JSON.stringify(msg)));
      });
    });
    ...
    describe('headers', () => {
      ...
      it('should send packet headers', () => {
        ...
        client['publish'](msg, () => {
          expect(sendToQueueStub.getCall(0).args[2].headers).to.eql(requestHeaders);
        });
      });
  ...
  describe('dispatchEvent', () => {
    ...
    it('should throw error', async () => {
      ...
      client['dispatchEvent'](msg).catch((err) => expect(err).to.be.instanceOf(Error));
    });
