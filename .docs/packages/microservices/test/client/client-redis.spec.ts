...
describe('ClientRedis', () => {
  ...
  describe('publish', () => {
    ...
    describe('dispose callback', () => {
      ...
      beforeEach(async () => {
        ...
        getReplyPatternStub = sinon.stub(client, 'getReplyPattern').callsFake(() => channel);
  ...
  describe('createResponseCallback', () => {
    ...
    describe('not completed', () => {
      beforeEach(async () => {
        ...
        await subscription('channel', Buffer.from(JSON.stringify(responseMessage)));
      });
  ...
  describe('registerReconnectListener', () => {
    it('should bind reconnect event handler', () => {
      ...
      expect(callback.getCall(0).args[0]).to.be.eql(RedisEventsMap.RECONNECTING);
    });
  });
