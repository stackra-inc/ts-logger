...
> {
  ...
  public bindListeners(
    ...
    controllers.forEach((wrapper) =>
      this.listenersController.registerPatternHandlers(wrapper, serverInstance, moduleName)
    );
  }

  public bindClients(items: Map<string | symbol | Function, InstanceWrapper<unknown>>) {
    items.forEach(({ instance, isNotMetatype }) => {
      !isNotMetatype && this.listenersController.assignClientsToProperties(instance as object);
    });
  }
