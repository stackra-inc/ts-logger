...
import { NATS_DEFAULT_GRACE_PERIOD, NATS_DEFAULT_URL, NO_MESSAGE_HANDLER } from '../constants';
...
> extends Server<E, S> {
  ...
  private async waitForGracePeriod() {
    const gracePeriod = this.getOptionsProp(this.options, 'gracePeriod', NATS_DEFAULT_GRACE_PERIOD);
  ...
  public async handleMessage(channel: string, natsMsg: NatsMsg) {
    ...
    const publish = this.getPublisher(natsMsg, (message as IncomingRequest).id, natsCtx);
    ...
    for await (const status of client.status()) {
      ...
      switch (status.type) {
        ...
        case 'disconnect':
          ...
          this.statusEventEmitter.emit(NatsEventsMap.DISCONNECT, status.data as string);
          break;
        ...
        case 'reconnect':
          ...
          this._status$.next(NatsStatus.CONNECTED as S);
          this.statusEventEmitter.emit(NatsEventsMap.RECONNECT, status.data as string);
          break;
  ...
  public on<EventKey extends keyof E = keyof E, EventCallback extends E[EventKey] = E[EventKey]>(
    event: EventKey,
