...
import { NO_MESSAGE_HANDLER, REDIS_DEFAULT_HOST, REDIS_DEFAULT_PORT } from '../constants';
import { RedisContext } from '../ctx-host';
import { Transport } from '../enums';
import { RedisEvents, RedisEventsMap, RedisStatus } from '../events/redis.events';
...
export class ServerRedis extends Server<RedisEvents, RedisStatus> {
  ...
  public getMessageHandler(pub: Redis) {
    return this.options?.wildcards
      ...
      : (channel: string, buffer: string) => this.handleMessage(channel, buffer, pub, channel);
  }

  public async handleMessage(channel: string, buffer: string, pub: Redis, pattern: string) {
    ...
    const publish = this.getPublisher(pub, channel, (packet as IncomingRequest).id, redisCtx);
  ...
  public registerReconnectListener(client: { on: (event: string, fn: () => void) => void }) {
  ...
  public registerReadyListener(client: { on: (event: string, fn: () => void) => void }) {
  ...
  public registerEndListener(client: { on: (event: string, fn: () => void) => void }) {
    client.on('end', () => {
      if (this.isManuallyClosed) {
        return;
      }
