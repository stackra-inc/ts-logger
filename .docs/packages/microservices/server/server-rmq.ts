...
import { IncomingRequest, OutgoingResponse, ReadPacket } from '../interfaces/packet.interface';
...
export class ServerRMQ extends Server<RmqEvents, RmqStatus> {
  ...
  public async setupChannel(channel: Channel, callback: Function) {
    ...
    if (this.options.exchange || this.options.wildcards) {
      ...
      if (this.options.wildcards) {
        ...
        await Promise.all(
          routingKeys.map((routingKey) => channel.bindQueue(createdQueue, exchange, routingKey))
        );
  ...
  public async handleMessage(message: Record<string, any>, channel: any): Promise<void> {
    if (isNil(message)) {
      return;
    }
    ...
    return this.onProcessingStartHook(this.transportId, rmqContext, async () => {
      ...
      const publish = <T>(data: T) =>
        this.sendMessage(data, properties.replyTo, properties.correlationId, rmqContext);
  ...
  protected initializeWildcardHandlersIfExist() {
    ...
    handlers.forEach((handler, pattern) => {
      if (pattern.includes(RMQ_WILDCARD_ALL) || pattern.includes(RMQ_WILDCARD_SINGLE)) {
  ...
  private matchRmqPattern(pattern: string, routingKey: string): boolean {
    ...
    for (const [i, currentPattern] of patternSegments.entries()) {
      ...
      if (currentPattern !== RMQ_WILDCARD_SINGLE && currentPattern !== currentRoutingKey) {
        return false;
      }
    }
