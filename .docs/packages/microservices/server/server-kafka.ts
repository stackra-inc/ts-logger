...
export class ServerKafka extends Server<never, KafkaStatus> {
  ...
  protected registerConsumerEventListeners() {
    ...
    this.consumer.on(this.consumer.events.CONNECT, () => this._status$.next(KafkaStatus.CONNECTED));
    ...
    this.consumer.on(this.consumer.events.STOP, () => this._status$.next(KafkaStatus.STOPPED));
    this.consumer.on(this.consumer.events.CRASH, () => this._status$.next(KafkaStatus.CRASHED));
  }
  ...
  protected registerProducerEventListeners() {
    ...
    this.producer.on(this.producer.events.CONNECT, () => this._status$.next(KafkaStatus.CONNECTED));
  ...
  public createClient<T = any>(): T {
    return new kafkaPackage.Kafka(
      Object.assign({ logCreator: KafkaLogger.bind(null, this.logger) }, this.options.client, {
  ...
  public getPublisher(
    ...
    return (data: any) =>
      this.sendMessage(data, replyTopic, replyPartition, correlationId, context);
  }
  ...
  public assignReplyPartition(replyPartition: string | null | undefined, outgoingMessage: Message) {
