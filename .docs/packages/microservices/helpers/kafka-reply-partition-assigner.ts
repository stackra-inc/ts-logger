...
export class KafkaReplyPartitionAssigner {
  ...
  constructor(
    ...
    kafkaPackage = loadPackage('kafkajs', KafkaReplyPartitionAssigner.name, () =>
      require('kafkajs')
    );
  }
  ...
  public async assign(group: {
    ...
    sortedMemberIds.forEach((assignee) => {
      ...
      group.topics.forEach((topic) => {
        ...
        if (previousAssignment[assignee] && !isUndefined(previousAssignment[assignee][topic])) {
  ...
  public protocol(subscription: { topics: string[]; userData: Buffer }): GroupState {
