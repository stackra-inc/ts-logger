...
import { MqttRecord, MqttRecordOptions } from '../record-builders/mqtt.record-builder';
...
export class ClientMqtt extends ClientProxy<MqttEvents, MqttStatus> {
  ...
  public mergeCloseEvent<T = any>(instance: MqttClient, source$: Observable<T>): Observable<T> {
  ...
  protected dispatchEvent(packet: ReadPacket): Promise<any> {
    ...
    const options =
      isObject(packet?.data) && packet.data instanceof MqttRecord ? packet.data.options : undefined;
    delete packet?.data?.options;
