...
export class ClientRedis extends ClientProxy<RedisEvents, RedisStatus> {
  ...
  public async connect(): Promise<any> {
    ...
    this.connectionPromise = Promise.all([this.subClient.connect(), this.pubClient.connect()]);
  ...
  public registerErrorListener(client: Redis) {
    client.addListener(RedisEventsMap.ERROR, (err: any) => this.logger.error(err));
  }
  ...
  public createResponseCallback(): (channel: string, buffer: string) => Promise<void> {
