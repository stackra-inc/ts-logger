...
export class ClientRMQ extends ClientProxy<RmqEvents, RmqStatus> {
  ...
  public async setupChannel(channel: Channel, resolve: Function) {
    const prefetchCount =
      this.getOptionsProp(this.options, 'prefetchCount') || RQM_DEFAULT_PREFETCH_COUNT;
  ...
  public registerErrorListener(client: AmqpConnectionManager): void {
    client.addListener(RmqEventsMap.ERROR, (err: any) => this.logger.error(err));
  }
  ...
  public async handleMessage(
    packet: unknown,
    options: Record<string, unknown> | ((packet: WritePacket) => any) | undefined,
    callback?: (packet: WritePacket) => any
  ): Promise<void> {
  ...
  protected publish(message: ReadPacket, callback: (packet: WritePacket) => any): () => void {
    try {
      ...
      const listener = ({
        ...
        content: Buffer;
        options: Record<string, unknown>;
      }) => this.handleMessage(this.parseMessageContent(content), options, callback);
      ...
      const sendOptions = {
        ...
        persistent: this.getOptionsProp(this.options, 'persistent', RQM_DEFAULT_PERSISTENT),
        ...options,
      ...
      if (this.options.wildcards || this.options.exchangeType === 'fanout') {
        ...
        this.channel!.publish(exchange, stringifiedPattern, content, sendOptions).catch((err) =>
          callback({ err })
        );
      } else {
  ...
  protected dispatchEvent(packet: ReadPacket): Promise<any> {
    ...
    return new Promise<void>((resolve, reject) => {
      ...
      const errorCallback = (err: unknown) => (err ? reject(err as Error) : resolve());
