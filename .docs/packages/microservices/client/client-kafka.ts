...
import { throwError as _throw, connectable, defer, Observable, Subject } from 'rxjs';
import { mergeMap } from 'rxjs/operators';
import { KAFKA_DEFAULT_BROKER, KAFKA_DEFAULT_CLIENT, KAFKA_DEFAULT_GROUP } from '../constants';
...
import { KafkaLogger, KafkaParser, KafkaReplyPartitionAssigner } from '../helpers';
import {
  ClientKafkaProxy,
  KafkaOptions,
  MsPattern,
...
import { KafkaRequest, KafkaRequestSerializer } from '../serializers/kafka-request.serializer';
 ...
 * @publicApi
 */
export class ClientKafka extends ClientProxy<never, KafkaStatus> implements ClientKafkaProxy {
  ...
  public commitOffsets(topicPartitions: TopicPartitionOffsetAndMetadata[]): Promise<void> {
    if (this._consumer) {
  ...
  protected registerConsumerEventListeners() {
    ...
    this._consumer.on(this._consumer.events.STOP, () => this._status$.next(KafkaStatus.STOPPED));
    this.consumer.on(this._consumer.events.CRASH, () => this._status$.next(KafkaStatus.CRASHED));
  }
