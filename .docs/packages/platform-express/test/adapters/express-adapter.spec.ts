...
describe('ExpressAdapter', () => {
  ...
  describe('registerParserMiddleware', () => {
    it('should register the express built-in parsers for json and urlencoded payloads', () => {
      ...
      const jsonParserSpy = sinon.stub(express, 'json').returns(jsonParserInstance);
      const urlencodedParserSpy = sinon.stub(express, 'urlencoded').returns(urlencodedInstance);
      ...
      expect(useSpy.calledWith(sinon.match.same(jsonParserInstance))).to.be.true;
      expect(useSpy.calledWith(sinon.match.same(urlencodedInstance))).to.be.true;
