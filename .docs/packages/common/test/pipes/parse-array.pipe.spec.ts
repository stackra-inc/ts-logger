...
import { IsBoolean, IsDate, IsDefined, IsNumber, IsString, ValidateNested } from 'class-validator';
  ...
  describe('transform', () => {
    describe('when undefined value', () => {
      describe('and optional disabled', () => {
        it('should throw an exception', async () => {
          ...
          return expect(target.transform(undefined, {} as ArgumentMetadata)).to.to.be.rejectedWith(
            BadRequestException
          );
        });
      });
      describe('and optional enabled', () => {
        it('should return undefined', async () => {
          ...
          expect(await target.transform(undefined, {} as ArgumentMetadata)).to.be.undefined;
        });
      });
    });
    ...
    describe('when value is not parseable', () => {
      ...
      it('should throw an exception (boolean)', async () => {
        return expect(target.transform(true, {} as ArgumentMetadata)).to.be.rejectedWith(
          BadRequestException
        );
      });
      it('should throw an exception (number)', async () => {
        return expect(target.transform(3, {} as ArgumentMetadata)).to.be.rejectedWith(
          BadRequestException
        );
      });
      it('should throw an exception (object)', async () => {
        return expect(target.transform({}, {} as ArgumentMetadata)).to.be.rejectedWith(
          BadRequestException
        );
      });
      ...
      describe('and "optional" is enabled', () => {
        it('should throw an exception', async () => {
          ...
          return expect(pipe.transform({}, {} as ArgumentMetadata)).to.be.rejectedWith(
            BadRequestException
          );
        });
      });
    ...
    describe('when value is parseable (string)', () => {
      it('should parse an array based on the separator', async () => {
        ...
        expect(
          await target.transform('1,2.0,3,{},true,null,,', {} as ArgumentMetadata)
        ).to.be.deep.equal(['1', '2.0', '3', '{}', 'true', 'null', '', '']);

        target = new ParseArrayPipe({ separator: '/' });

        expect(await target.transform('1/2/3', {} as ArgumentMetadata)).to.be.deep.equal([
          '1',
          '2',
          '3',
        ]);
      ...
      describe('and type is specified', () => {
        it('should parse & validate the array', async () => {
          ...
          try {
            ...
            expect(err.getResponse().message).to.deep.equal('[2] item must be a number');
          }
          ...
          try {
            ...
            expect(err).to.be.instanceOf(BadRequestException);
            expect(err.getResponse().message).to.deep.equal('[0] item must be a boolean value');
          }
    ...
    describe('when items type is determined', () => {
      ...
      it('should validate and transform each item', async () => {
        ...
        let items = await target.transform([{}, {}, {}], {} as ArgumentMetadata);
        items.forEach((item) => {
        ...
        expect(await target.transform('1,2.0,3', {} as ArgumentMetadata)).to.deep.equal([1, 2, 3]);
        ...
        expect(await target.transform('true,false', {} as ArgumentMetadata)).to.deep.equal([
          true,
          false,
        ]);
      });
        ...
        it('should validate each item and concat errors', async () => {
          ...
          try {
            await pipe.transform(
              [{ number: '1' }, { number: '1' }, { number: 1 }] as ArrItemWithProp[],
              {} as ArgumentMetadata
            );
          } catch (err) {
            expect(err).to.be.instanceOf(BadRequestException);
