  ...
  describe('transform', () => {
    ...
    describe('when validation fails', () => {
      ...
      it('should throw an error - not a string', async () => {
        target = new ParseUUIDPipe({ exceptionFactory });
        await expect(target.transform(undefined!, {} as ArgumentMetadata)).to.be.rejectedWith(
          TestException
        );
      });
      ...
      it('should throw an error - v3', async () => {
        ...
        await expect(target.transform(v4, {} as ArgumentMetadata)).to.be.rejectedWith(
          TestException
        );
        await expect(target.transform(v5, {} as ArgumentMetadata)).to.be.rejectedWith(
          TestException
        );
      });
      ...
      it('should throw an error - v4', async () => {
        ...
        await expect(target.transform(v3, {} as ArgumentMetadata)).to.be.rejectedWith(
          TestException
        );
        await expect(target.transform(v5, {} as ArgumentMetadata)).to.be.rejectedWith(
          TestException
