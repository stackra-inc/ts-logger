...
describe('ClassSerializerInterceptor', () => {
  ...
  describe('intercept', () => {
    ...
    it('should transform array of objects', (done) => {
      ...
      mockReflector.getAllAndOverride.returns(undefined);
      mockTransformerPackage.classToPlain.onFirstCall().returns(transformedItem1);
      mockTransformerPackage.classToPlain.onSecondCall().returns(transformedItem2);
      (mockCallHandler.handle as sinon.SinonStub).returns(of(response));
  ...
  describe('transformToPlain', () => {
    ...
    it('should use classToPlain when no type option provided', () => {
      ...
      expect(result).to.equal(output);
      expect(mockTransformerPackage.classToPlain.calledOnceWith(input, {})).to.be.true;
    ...
    it('should convert plain to instance then to plain when type provided but not matching', () => {
      ...
      expect(mockTransformerPackage.plainToInstance.calledOnceWith(UserDto, plainInput, options)).to
        .be.true;
      expect(mockTransformerPackage.classToPlain.calledOnceWith(instanceOutput, options)).to.be
        .true;
    });
  ...
  describe('edge cases and error handling', () => {
    ...
    it('should handle array with mixed types', () => {
      const input = [{ id: 1, name: 'Test' }, null, undefined, { id: 2, name: 'Test2' }];

      mockTransformerPackage.classToPlain.onCall(0).returns({ id: 1, name: 'Test' });
      mockTransformerPackage.classToPlain.onCall(1).returns(null);
      mockTransformerPackage.classToPlain.onCall(2).returns(undefined);
      mockTransformerPackage.classToPlain.onCall(3).returns({ id: 2, name: 'Test2' });
