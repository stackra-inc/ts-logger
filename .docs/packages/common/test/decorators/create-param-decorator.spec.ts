...
describe('createParamDecorator', () => {
  ...
  describe('returned decorator', () => {
    ...
    describe('when > 0 pipes have been passed', () => {
      ...
      it('should enhance param with ParseIntPipe', () => {
        const metadata = Reflect.getMetadata(ROUTE_ARGS_METADATA, Test, 'testNoData');
        const key = Object.keys(metadata)[0];
        expect(metadata[key]).to.be.eql({
          data: undefined,
          factory: factoryFn,
      ...
      it('should enhance param with ParseIntPipe metatype', () => {
        const metadata = Reflect.getMetadata(ROUTE_ARGS_METADATA, Test, 'testNoDataClass');
        const key = Object.keys(metadata)[0];
        expect(metadata[key]).to.be.eql({
          data: undefined,
          factory: factoryFn,
  ...
  describe('returned generic typed decorator', () => {
    ...
    const stringOrNumberDecorator = createParamDecorator<string | number>(factoryFn);
