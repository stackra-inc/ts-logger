...
describe('ConfigurableModuleBuilder', () => {
  describe('setExtras', () => {
    ...
    it('should preserve extras in registerAsync transformation', () => {
      ...
      const { ConfigurableModuleClass } = new ConfigurableModuleBuilder()
        .setExtras({ folder: 'default' }, (definition, extras: { folder: string }) => {
      ...
      expect(asyncResult).to.have.property('customProperty', 'folder: forRootAsync');
    });
  });
  ...
  describe('build', () => {
    it('should return a fully typed "ConfigurableModuleClass"', () => {
      ...
      try {
        ...
        expect(err.message).to.equal('"ASYNC_OPTIONS_TYPE" is not supposed to be used as a value.');
      }
      try {
        expect(OPTIONS_TYPE.isGlobal).to.equal(undefined);
      } catch (err) {
        expect(err).to.be.instanceOf(Error);
        expect(err.message).to.equal('"OPTIONS_TYPE" is not supposed to be used as a value.');
      }
    });
  });
});
