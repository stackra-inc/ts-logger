...
import { ErrorHttpStatusCode, HttpErrorByCode } from '../utils/http-error-by-code.util';
...
export class ParseEnumPipe<T = any> implements PipeTransform<T> {
  ...
  protected isEnum(value: T): boolean {
    const enumValues = Object.keys(this.enumType as object).map((item) => this.enumType[item]);
